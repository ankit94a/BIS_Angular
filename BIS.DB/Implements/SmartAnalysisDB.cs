...
namespace BIS.DB.Implements
{
	public class SmartAnalysisDB : ISmartAnalysisDB
	{
		...
		public async Task<MeanValueModel> GetEntries(long corpsId, long divisionId, RoleType roleType, FilterModelEntries filterModel)
		{
			...
			// handling filter arrays 
			//if (filterModel != null && filterModel.Frmn?.Count > 0)
			//{
			//	query = query.Where(ms => filterModel.Indicator.Contains(ms.Indicator));
			//}
			if (filterModel != null && filterModel.Sector?.Count > 0)
		...
		//	var groupedQuery = new List<dynamic>();

		//	if (filterModel.FilterType == FilterType.Daily)
		//	{
		//		groupedQuery = await query
		//			.GroupBy(ms => ms.CreatedOn.Value.Date)  // Group by day
		//			.Select(group => new
		//			{
		//				Date = group.Key,
		//				Count = group.Count()
		...
		//	{
		//		groupedQuery = await query
		//			.GroupBy(ms => new { Year = ms.CreatedOn.Value.Year, Week = GetWeekOfYear(ms.CreatedOn.Value) })  // Group by year and week
		//			.Select(group => new
		//			{
		//				Date = $"{group.Key.Year}-W{group.Key.Week:D2}",  // Format as "YYYY-WW"
		//				Count = group.Count()
		//			})
		//			.ToListAsync();
		//	}
		...
		//	{
		//		groupedQuery = await query
		//			.GroupBy(ms => new { Year = ms.CreatedOn.Value.Year, Month = ms.CreatedOn.Value.Month })  // Group by year and month
		//			.Select(group => new
		//			{
		//				Date = $"{group.Key.Year}-{group.Key.Month:D2}",  // Format as "YYYY-MM"
		//				Count = group.Count()
		//			})
		//			.ToListAsync();
		//	}
